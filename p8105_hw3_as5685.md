...
is Bulgarian Yogurt in Yougurt aisle. `product_name` and `aisle`
indicate the name of specific product and which aisle it is placed in
the grocery which make easier for sorting them. such as Fresh Dill
belongs to fresh herbs. Some other variables such as `order_dow`.
`order_hour_od_day` and `reordered` are helpful for staff to do some
market research.
...
Soda` with 387 ordered. The most popular three items in `dog food care`
are `rganix Chicken & Brown Rice Recipe` with 28 ordered `Small dog
biscuits`with `Snack Sticks Chicken & Rice Recipe Dog
Treats`
